...
async function broadcastViaBackend(signedhex) {
  const r = await axios.post(`${API_URL}/broadcast`, { signedhex });
  return r.data?.txid;
}
...
async function signAndBroadcastInscription(wallet, tx) {
  const commit = await signAndBroadcastCommit(wallet, tx);
  let revealTxid = null;
  if (tx.signed_reveal_rawtransaction) {
    revealTxid = await broadcastViaBackend(tx.signed_reveal_rawtransaction);
  }
    ...
    try {
      ...
      if (mode === 'single') {
        ...
        for (let i = 0; i < data.transactions.length; i++) {
          ...
          let broadcastResult;
          try {
            broadcastResult = await signAndBroadcastInscription(wallet, item.tx);
          } catch (sigErr) {
        ...
        let broadcastResult;
        try {
          broadcastResult = await signAndBroadcastInscription(wallet, data.tx);
        } catch (sigErr) {
          broadcastResult = { error: sigErr.message };
        }
